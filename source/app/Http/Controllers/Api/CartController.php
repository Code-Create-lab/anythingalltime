...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if ($cart_items) {
            ...
            if ($store_id != $store_id1) {
                $message = ['status' => '2', 'message' => 'your previous cart items will be cleared when you are going to order from new store.'];
                ...
                $ph = DB::table('users')
                    ...
                    ->first();
                if (! $ph) {
                    $message = ['status' => '0', 'message' => 'User not Found'];
                ...
                if ($qty > $p->max_ord_qty) {
                    ...
                    $message = ['status' => '0', 'message' => 'you have to order '.$p_name.' quantity between '.$p->min_ord_qty.' to '.$p->max_ord_qty];

                    return $message;
                }
                if ($p->stock < $qty) {
                    $message = ['status' => '0', 'message' => 'only '.$p->stock.' available in stock'];
                ...
                if ($insert) {
                    ...
                    $tax_per_avg = ($sum->count != 0) ? $sum->sum_per / $sum->count : 0;
                    $round_pr = round($tax_per_avg, 2);
                    $taxtotal = round($sum->sum_tax, 2);
                    ...
                    foreach ($cart_items1 as $cart_itemss) {
                        ...
                        if ($deal) {
                            ...
                            $new_price = round($deal->deal_price, 2) * $cart_itemss->cart_qty;
                            $cur_price = round($cart_itemss->ord_price, 2);
                        } else {
                            ...
                            $new_price = round($sp->price, 2) * $cart_itemss->cart_qty;
                            $cur_price = round($cart_itemss->ord_price, 2);
                        }
                        ...
                        if (count($getrating) > 0) {
                            ...
                            $rating = DB::table('product_rating')
                                ...
                                ->where('store_id', $store_id)
                                ->avg('rating');
                            $cart_itemss->avgrating = round($rating, 2);
                        ...
                        if ($cart_itemss->total_mrp != 0) {
                            ...
                            $cart_itemss->discountper = round($discountper, 2);
                        } else {
                    ...
                    $adata = ['discountonmrp' => $discountonmrp, 'total_price' => $sum1, 'total_mrp' => $mrppp, 'total_items' => $sum->count, 'store_details' => $nearbystore, 'total_tax' => $taxtotal, 'avg_tax' => $round_pr, 'data' => $cart_items1];

                    $message = ['status' => '1', 'message' => 'Successfully added to cart', 'data' => $adata];

                    return $message;
                } else {
                    $message = ['status' => '0', 'message' => 'insertion failed'];
            ...
            // Add validation for max order quantity and stock
            if ($qty > $p->max_ord_qty) {
                $p_name = $p->product_name.'('.$p->quantity.$p->unit.')*'.$qty;
                $message = ['status' => '0', 'message' => 'you have to order '.$p_name.' quantity between '.$p->min_ord_qty.' to '.$p->max_ord_qty];
            ...
            if ($insert) {
                ...
                $cart_items1 = DB::table('store_orders')
                    ...
                    ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                    ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                    ->select('store_orders.product_name', 'store_orders.varient_image', 'store_orders.quantity', 'store_orders.unit', 'store_products.price', 'store_orders.qty as cart_qty', 'store_orders.total_mrp', 'store_orders.order_cart_id', 'store_orders.order_date', 'store_orders.store_approval', 'store_orders.store_id', 'store_orders.varient_id', 'product.product_id', 'store_products.stock', 'store_orders.tx_per', 'store_orders.price_without_tax', 'store_orders.tx_price', 'store_orders.tx_name', 'product.product_image', 'product_varient.description', 'product.type', 'store_orders.price as ord_price')
                ...
                $message = ['status' => '1', 'message' => 'Cart Updated', 'data' => $adata];
    ...
    public function make_an_order(Request $request)
    {
        ...
        if ($store) {
            ...
            return ['status' => '0', 'message' => 'No Items in cart'];
        }
        ...
        if (! $typessss) {
            return ['status' => '0', 'message' => 'you do not have selected an address in delivery range of store please add/select an address in store delivery range'];
        }
        ...
        $ar_id = DB::table('service_area')
            ...
            ->first();
        if (! $ar) {
            $message = ['status' => '0', 'message' => 'Select any Address'];
        ...
        if ($min) {
            if ($pricecheck < $min->min_value) {
                $message = ['status' => '0', 'message' => 'you have to order between '.$min->min_value.' to '.$min->max_value];
        ...
        if ($time_slot == 'instant') {
            ...
            if ($checkmem) {
                ...
                $message = ['status' => '5', 'message' => 'Please buy a membership for use instant delivery/free delivery feature'];
        ...
        if ($insert) {
            $oo = DB::table('orders')
                ->insertGetId(['cart_id' => $cart_id,
                    ...
                    'user_id' => $user_id,
                    'store_id' => $store_id,
                    'rem_price' => round(($price2 + $charge), 2),
                    'order_date' => $created_at,
            ...
            $message = ['status' => '1', 'message' => 'Proceed to payment', 'data' => $ordersuccessed];
    ...
    public function show_cart(Request $request)
    {
        ...
        if (count($cart_items) > 0) {
            ...
            $message = ['status' => '1', 'message' => 'cart_items', 'data' => $adata];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'No Items in Cart'];
    ...
    public function del_frm_cart(Request $request)
    {
        ...
        if ($cart_items) {
            ...
            $tax_per_avg = ($sum->count != 0) ? $sum->sum_per / $sum->count : 0;
            $round_pr = round($tax_per_avg, 2);
            $taxtotal = round($sum->sum_tax ?? 0, 2);

            $datee = date('Y-m-d');

            $sum1 = round($sum->sum ?? 0, 2);
            $discountonmrp = ($sum->mrp ?? 0) - $sum1;
            $discountonmrp = round($discountonmrp, 2);

            // Always define $cart_items1
            $cart_items1 = DB::table('store_orders')
                ->join('store_products', 'store_orders.varient_id', '=', 'store_products.varient_id')
                ->join('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                ->join('product', 'product_varient.product_id', '=', 'product.product_id')
                ...
                ->where('store_orders.order_cart_id', 'incart')
                ->get();
            if ($cart_items1->isEmpty()) {
                $cart_items1 = [];
            }

            $data = ['discountonmrp' => $discountonmrp, 'total_price' => $sum1, 'total_items' => $sum->count, 'data' => $cart_items2, 'total_tax' => $taxtotal, 'avg_tax' => $round_pr, 'data' => $cart_items1];

            $message = ['status' => '1', 'message' => 'Product has been removed from cart', 'data' => $data];
    ...
    public function check_cart(Request $request)
    {
        ...
        if (count($cart_items) > 0) {
            ...
            if ($store_id != $store_id1) {
                $message = ['status' => '1', 'message' => 'your previous cart items will be cleared when you are going to order from new store.'];

                return $message;

            } else {
                $message = ['status' => '0', 'message' => 'enter to store'];
    ...
    public function clear_cart(Request $request)
    {
        ...
        if ($cart_items) {
            $message = ['status' => '1', 'message' => 'your cart has been cleared.'];

            return $message;

        } else {
            $message = ['status' => '0', 'message' => 'nothing in cart'];
    ...
    public function re_ordercart(Request $request)
    {
        ...
        if ($orde) {
            ...
            $message = ['status' => '0', 'message' => 'No Cart Found'];
        ...
        if ($insert) {
            ...
            $message = ['status' => '1', 'message' => 'Added order Products to cart which are available in stock', 'data' => $adata];
