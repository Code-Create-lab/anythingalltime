...
class forgotpasswordController extends Controller
{
    public function checkotponoff(Request $request)
    {
        ...
        if ($check) {
            if ($check->msg91 == 0 && $check->twilio == 0 && $check->status == 0) {
                $message = ['status' => '0', 'message' => 'SMS/OTP are Off'];

                return $message;
            } else {
                $message = ['status' => '1', 'message' => 'SMS/OTP are On'];

                return $message;
            }
        } else {
            $message = ['status' => '2', 'message' => 'table or Data Not Found'];
    ...
    public function forgot_password(Request $request)
    {
        ...
        if ($check) {
            $data = ['to' => $user_email, 'from' => 'noreply@gogrocer.in', 'to-name' => $user_email, 'from-name' => $app_name];
            ...
            $message = ['status' => '1', 'message' => 'Email sent'];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'Email/phone is not registered'];
    ...
    public function checknum(Request $request)
    {
        ...
        if ($getUser) {
            ...
            $message = ['status' => 1, 'message' => 'Phone number is available in DB'];
