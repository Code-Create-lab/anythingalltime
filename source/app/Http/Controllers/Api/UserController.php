...
class UserController extends Controller
{
    ...
    public function social_login(Request $request)
    {
        ...
        if ($logintype == 'google') {
            ...
            if ($checkuser) {
                ...
                $message = ['status' => '1', 'message' => 'Login Successfully', 'data' => $checkuser, 'token' => $token];
                ...
                DB::table('notificationby')
                    ->insert(['user_id' => $Userreg,
                        ...
                        'app' => '1',
                        'email' => '1']);
                $message = ['status' => '2', 'message' => 'go to register details page', 'data' => $user_email];
            ...
            if ($checkuser) {
                ...
                $appsetting = DB::table('notificationby')
                    ->insert(['user_id' => $Userreg,
                        ...
                        'app' => '1',
                        'email' => '1']);
                $message = ['status' => '4', 'message' => 'go to register details page', 'apple_id' => $fb_id];
            ...
            if ($fb_id == null && $email != null) {
                ...
                $message = ['status' => '3', 'message' => 'go to register details page', 'fb_id' => null];
            ...
            if ($checkuser) {
                ...
                $message = ['status' => '3', 'message' => 'go to register details page', 'fb_id' => $fb_id];
    ...
    public function verifyotpfirebase(Request $request)
    {
        ...
        if ($getUser) {
            if ($status == 'success') {
                if ($ver == 0) {
                    if ($referral_code != null) {
                        ...
                        if ($getReferredUser1) {
                            ...
                            $message = ['status' => '0', 'message' => 'wrong referral code'];
                ...
                $message = ['status' => '1', 'message' => 'Phone Verified! login successfully', 'data' => $getUser, 'token' => $token];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Wrong OTPss'];
    ...
    public function register_details(Request $request)
    {
        ...
        if ($check) {
            ...
            $message = ['status' => '0', 'message' => 'User Not Founds'];
    ...
    public function forgotPassword(Request $request)
    {
        ...
        if ($checkUser) {
            ...
            if ($updateOtp) {
                ...
                $message = ['status' => '0', 'message' => 'Something wrong'];
    ...
    public function verifyOtpPass(Request $request)
    {
        ...
        if ($getUser) {
            ...
            if ($otp == $getotp) {
                $message = ['status' => '1', 'message' => 'Otp Matched Successfully', 'data' => $getUser];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Wrong OTP'];
    ...
    public function changePassword(Request $request)
    {
        ...
        if ($getUser) {
            ...
            if ($updateOtp) {
                ...
                $message = ['status' => '0', 'message' => 'Use Another Password'];
    ...
    public function profile_edit(Request $request)
    {
        ...
        if ($uu->user_phone == '9999999999') {
            $user = User::where('id', $user_id)
                ->first();
            $token = $user->createToken('token')->accessToken;
            $message = ['status' => '0', 'message' => 'You can not change the details for demo account'];
            ...
            if ($checkUser && $checkUser->is_verified == 1) {
                $message = ['status' => '0', 'message' => 'This Phone number is attached with another account'];
                ...
                if ($insertUser) {
                    ...
                    $message = ['status' => '0', 'message' => 'Nothing To Update'];
    ...
    public function user_block_check(Request $request)
    {
        ...
        if ($user) {
            if ($user->block == 1) {
                $message = ['status' => '1', 'message' => 'User is Blocked'];

                return $message;
            } else {
                $message = ['status' => '2', 'message' => 'User is Active'];
    ...
    public function resendotp(Request $request)
    {
        ...
        if ($getUser) {
            if ($firebase->status == 1) {
                ...
                $message = ['status' => '2', 'message' => 'Otp sent via firebase', 'data' => $getUser];
                ...
                $otpmsg = $this->otpmsg($otpval, $user_phone);
                $message = ['status' => '1', 'message' => 'Otp sent', 'data' => $getUser];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Otp Off', 'data' => $getUser];

                return $message;
            }

        } else {
            $message = ['status' => '0', 'message' => 'User not found', 'data' => $checuss];
    ...
    public function login_with_email(Request $request)
    {
        ...
        if ($checkUser) {
            ...
            if (Hash::check($user_password, $checkUserreg->password)) {
                ...
                $user->cart_count = $countp;
                $token = $user->createToken('token')->accessToken;
                $message = ['status' => '1', 'message' => 'login successfully', 'data' => $user, 'token' => $token];

                return $message;

            } else {
                $message = ['status' => '0', 'message' => 'Wrong Password'];

                return $message;
            }

        } else {
            $message = ['status' => '2', 'message' => 'User not Registered'];
