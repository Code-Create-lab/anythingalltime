...
class TimeslotController extends Controller
{
    public function timeslot(Request $request)
    {
        ...
        if (! $time_slot) {
            return ['status' => '0', 'message' => 'Store not found', 'data' => []];
        }
        ...
        if (strtotime($date) == strtotime($selected_date)) {
            if (strtotime($tes2) <= $currenttime) {
                $message = ['status' => '0', 'message' => 'Oops No time slot present', 'data' => $current_time];
            ...
            if ($last_Number != null) {
                ...
                for ($i = 1; $i < count($array_of_time1) - 1; $i++) {
                    ...
                    if ($orders > $totorders) {
                        ...
                        $new_array_of_time1[] = ['timeslot' => ''.$array_of_time1[$i].' - '.$array_of_time1[$i + 1], 'availability' => 'available'];

                    } else {

                        $new_array_of_time1[] = ['timeslot' => ''.$array_of_time1[$i].' - '.$array_of_time1[$i + 1], 'availability' => 'unavailable'];
                    }
            ...
            $message = ['status' => '0', 'message' => "You can't select the back date", 'data' => $current_time];
        ...
        if (count($new_array_of_time1) > 0) {
            ...
            $message = ['status' => '1', 'message' => 'Present time Slot', 'data' => $new_array_of_time1];
