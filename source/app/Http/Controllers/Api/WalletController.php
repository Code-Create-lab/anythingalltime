...
class WalletController extends Controller
{
    ...
    public function walletamount(Request $request)
    {
        ...
        if (! $wallet) {
            $message = ['status' => '0', 'message' => 'No user Found', 'data' => 0];

            return $message;
        }

        $wallet_amt = $wallet->wallet;
        $message = ['status' => '1', 'message' => 'Wallet_amount', 'data' => $wallet_amt];
    ...
    public function add_credit(Request $request)
    {
        ...
        if ($recharge_status == 'success') {
            ...
            if ($insert) {
                ...
                $message = ['status' => '1', 'message' => 'wallet recharged successfully'];
            ...
            $insert = DB::table('wallet_recharge_history')
                ->insert(['user_id' => $user_id,
                    ...
                    'payment_gateway' => $payment_gateway]);
            $message = ['status' => '0', 'message' => 'Failed! try again', 'data' => []];
    ...
    public function totalbill(Request $request)
    {
        ...
        if (count($orders) > 0) {
            ...
            $message = ['status' => '1', 'message' => 'data found', 'data' => $or];

            return $message;
        } else {
            $message = ['status' => '1', 'message' => 'data not found', 'data' => []];
