...
class RewardController extends Controller
{
    public function redeem(Request $request)
    {
        ...
        $rew = $rewards->rewards;
        if ($rew <= 0) {
            $message = ['status' => '0', 'message' => 'You have not get any rewards yet'];
            ...
            if ($upuser) {
                ...
                $message = ['status' => '1', 'message' => 'Rewards Redeemed'];
    ...
    public function rewardvalues(Request $request)
    {
        ...
        if ($redeem_points) {
            $message = ['status' => '1', 'message' => 'Rewards Point Values', 'data' => $redeem_points];
    ...
    public function rewardlines(Request $request)
    {
        ...
        if (! $cc) {
            return response()->json([
                'status' => '0',
                'message' => 'No reward points available',
                'line1' => '',
                'line2' => '',
            ]);
        }
        ...
        if ($cc) {
            $message = ['status' => '1', 'message' => 'Checkout Rewards lines', 'line1' => $text1, 'line2' => $text2];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'no rewards with this order'];
