...
class CurrencyController extends Controller
{
    public function currency(Request $request)
    {
        ...
        if ($currency) {
            $message = ['status' => '1', 'message' => 'currency', 'data' => $currency];
    ...
    public function gatewaysettings(Request $request)
    {
        ...
        if (! $paypal_status || ! $razorpay_status || ! $paystack_status || ! $stripe_status) {
            $message = ['status' => '0', 'message' => 'Required settings not found', 'data' => []];

            return $message;
        }

        $stripe = ['stripe_status' => $stripe_status->value ?? '0',
        ...
        $paypal = ['paypal_status' => $paypal_status->value ?? '0',
            'paypal_client_id' => $paypal_client->value ?? '',
            'paypal_secret' => $paypal_secret->value ?? '',
        ];

        $paystack = ['paystack_status' => $paystack_status->value ?? '0',
            'paystack_public_key' => $paystack_public_key->value ?? '',
            'paystack_secret_key' => $paystack_secret_key->value ?? ''];

        $razorpay = ['razorpay_status' => $razorpay_status->value ?? '0',
            'razorpay_secret' => $razorpay_secret->value ?? '',
            'razorpay_key' => $razorpay_key->value ?? ''];

        if ($currency) {
            $message = ['status' => '1', 'message' => 'Payment Gateways and Values', 'razorpay' => $razorpay, 'paypal' => $paypal, 'stripe' => $stripe, 'paystack' => $paystack];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'No Payment Gateway Found', 'data' => []];
