...
class WishlistController extends Controller
{
    ...
    public function add_to_wishlist(Request $request)
    {
        ...
        if (! $p) {
            $message = ['status' => '0', 'message' => 'Product Variant not found'];

            return $message;
        }

        if ($p->deal_price != null && $p->valid_from < $current && $p->valid_to > $current) {
            $price = round($p->deal_price, 2);
        } else {
        ...
        if (! $check) {
            ...
            if ($insert) {
                $count = DB::table('wishlist')
                    ...
                    ->where('store_id', $store_id)
                    ->count();
                $message = ['status' => '1', 'message' => 'Added to Wishlist', 'wishlist_count' => $count];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Something Wents Wrong'];
            ...
            if ($del) {
                $count = DB::table('wishlist')
                    ...
                    ->where('store_id', $store_id)
                    ->count();
                $message = ['status' => '2', 'message' => 'Removed from Wishlist', 'wishlist_count' => $count];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Something Wents Wrong', 'data' => []];
    ...
    public function wishlist_to_cart(Request $request)
    {
        ...
        if ($store) {
            ...
            $message = ['status' => '1', 'message' => 'Added to cart', 'data' => $adata];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'All wishlisted items is out of stock'];
    ...
    public function show_wishlist(Request $request)
    {
        ...
        if (! $price1) {
            $message = ['status' => '0', 'message' => 'Nothing in Wishlist From This Location'];
        ...
        foreach ($wishlist_item as $store) {
            ...
            if (count($getrating) > 0) {
                ...
                $rating = DB::table('product_rating')
                    ...
                    ->where('store_id', $store_id)
                    ->avg('rating');
                $store->avgrating = round($rating, 2);
            ...
            if ($store->mrp != 0) {
                ...
                $store->discountper = round($discountper, 2);
            } else {
                $store->discountper = 0;
            }
        ...
        if ($wishlist_items != null) {
            ...
            $message = ['status' => '1', 'message' => 'Wishlist items', 'count' => $count, 'data' => $wishlist_items];
