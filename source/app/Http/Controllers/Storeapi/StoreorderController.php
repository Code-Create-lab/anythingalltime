...
class StoreorderController extends Controller
{
    ...
    public function nextdayorders(Request $request)
    {
        ...
        $ord = DB::table('orders')
            ...
            ->orderByRaw(DB::getDriverName() === 'sqlite' ?
                "CASE order_status WHEN 'Pending' THEN 1 WHEN 'Confirmed' THEN 2 WHEN 'Out_For_Delivery' THEN 3 WHEN 'Completed' THEN 4 ELSE 5 END ASC" :
                "FIELD(order_status , 'Pending', 'Confirmed', 'Out_For_Delivery', 'Completed') ASC")
            ->get();
        ...
        if (count($ord) > 0) {
            foreach ($ord as $ords) {
                ...
                $data[] = ['paid_by_wallet' => $ords->paid_by_wallet, 'coupon_discount' => $ords->coupon_discount, 'delivery_charge' => $ords->delivery_charge, 'user_address' => $ords->house_no.','.$ords->society.','.$ords->city.','.$ords->landmark.','.$ords->state.','.$ords->pincode, 'cart_id' => $cart_id, 'user_name' => $ords->name, 'user_phone' => $ords->user_phone,
            ...
            $data[] = ['order_details' => 'no orders found'];
        }
    ...
    public function productcancelled(Request $request)
    {
        ...
        if (count($orders) == 1 || count($orders) == 0) {
            ...
            $data[] = ['result' => 'product cancelled successfully'];
    ...
    public function order_rejected(Request $request)
    {
        ...
        $cart_update = DB::table('store_orders')
            ->where('order_cart_id', $cart_id)
            ->update(['store_approval' => 1]);
        $data[] = ['result' => 'Order Rejected successfully'];
    ...
    public function storeconfirm(Request $request)
    {
        ...
        $del_boy = DB::table('delivery_boy')
            ->select('boy_name', 'boy_phone', 'dboy_id', DB::raw('6371 * acos(cos(radians('.$store->lat.'))
        * cos(radians(lat))
        ...
        * sin(radians(lat))) AS distance'))
        ...
        foreach ($v as $vs) {
            ...
            if ($stoc) {
                ...
                $message = ['status' => '0', 'message' => $pr->product_name.'('.$pr->quantity.' '.$pr->unit.') is not available in your product list'];
        ...
        if ($del_boy) {
            ...
            if ($orderconfirm) {
                ...
                $message = ['status' => '1', 'message' => 'order is confirmed'];
            ...
            $message = ['status' => '0', 'message' => 'No Delivery Boy in Your City'];
