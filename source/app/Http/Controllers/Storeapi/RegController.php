...
class RegController extends Controller
{
    ...
    public function regstore(Request $request)
    {
        ...
        if ($checkall && $checkall->admin_approval == 0) {
            $message = ['status' => '0', 'message' => 'This Phone Number and Email Are Already Registered With Another Store and waiting for Admin Approval'];

            return $message;
        } elseif ($chkstorphon && $chkstorphon->admin_approval == 0) {
            $message = ['status' => '0', 'message' => 'This Phone Number is Already Registered With Another Store and waiting for Admin Approval'];

            return $message;
        } elseif ($chkstoremail && $chkstoremail->admin_approval == 0) {
            $message = ['status' => '0', 'message' => 'This Email is Already Registered With Another Store and waiting for Admin Approval'];

            return $message;
        } elseif ($checkall && $checkall->admin_approval == 1) {
            $message = ['status' => '0', 'message' => 'This Phone Number and Email Are Already Registered With Another Store.'];

            return $message;
        } elseif ($chkstorphon && $chkstorphon->admin_approval == 0) {
            $message = ['status' => '0', 'message' => 'This Phone Number is Already Registered With Another Store.'];

            return $message;
        } elseif ($chkstoremail && $chkstoremail->admin_approval == 0) {
            $message = ['status' => '0', 'message' => 'This Email is Already Registered With Another Store.'];
            ...
            if ($insert) {
                ...
                $message = ['status' => '1', 'message' => 'Store Registered, please wait for admin approval', 'data' => $store];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Something went wrong', 'data' => []];
    ...
    public function idlist(Request $request)
    {
        ...
        if (count($id) > 0) {
            $message = ['status' => '1', 'message' => 'id list', 'data' => $id];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'no id list found'];
