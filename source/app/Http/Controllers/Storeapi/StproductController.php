...
class StproductController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if ($prod != null) {
            ...
            $message = ['status' => '1', 'message' => 'Store Products', 'data' => $prod];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'Products not found', 'data' => []];
    ...
    public function category_list(Request $request)
    {
        ...
        $category = DB::table('categories')
            ...
            ->get();
        if (count($category) > 0) {
            $message = ['status' => '1', 'message' => 'Category List', 'data' => $category];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'Category not found', 'data' => []];
    ...
    public function st_addproduct(Request $request)
    {
        ...
        if ($insertproduct) {
            ...
            $message = ['status' => '1', 'message' => trans('keywords.Added Successfully')];

            return $message;

        } else {
            $message = ['status' => '0', 'message' => trans('keywords.Something Wents Wrong')];
    ...
    public function St_updateproduct(Request $request)
    {
        ...
        if ($insertproduct || $enternew) {
            $message = ['status' => '1', 'message' => trans('keywords.Updated Successfully')];
    ...
    {
        $store_product_id = $request->p_id;
        $product_id = $request->product_id;

        // If p_id is provided, it's deleting from store_products table
        // First find the store_products record to get the actual product info, then delete all related records
        if ($store_product_id) {
            $storeProduct = DB::table('store_products')->where('id', $store_product_id)->first();
            if ($storeProduct) {
                // Delete all store_products records for this product from this store
                $delete = DB::table('store_products')
                    ->where('store_id', $storeProduct->store_id)
                    ->where('p_id', $storeProduct->p_id)
                    ->delete();

                if ($delete) {
                    $message = ['status' => '1', 'message' => trans('keywords.Deleted Successfully')];
        ...
        // If product_id is provided, it's deleting the actual product
        if ($product_id) {
            $delete = DB::table('product')->where('product_id', $product_id)->delete();
            if ($delete) {
                $delete = DB::table('product_varient')->where('product_id', $product_id)->delete();
