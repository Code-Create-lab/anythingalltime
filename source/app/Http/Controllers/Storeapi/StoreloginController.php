...
class StoreloginController extends Controller
{
        ...
        if ($checkstore1) {
            ...
            if ($checkstore) {
                if ($checkstore->store_status == 0) {
                    $message = ['status' => '2', 'message' => 'Your store has been blocked please contact admin.', 'data' => []];
                    ...
                    if ($checkstore->admin_approval == 1) {
                        if (Hash::check($password, $checkstore->password)) {
                            ...
                            $message = ['status' => '1', 'message' => 'login successfully', 'data' => [$checkstore]];

                            return $message;
                        } else {
                            $message = ['status' => '0', 'message' => 'Wrong Password', 'data' => []];

                            return $message;
                        }
                    } else {
                        $message = ['status' => '0', 'message' => 'Your store is under approval. Please wait for admin approval.', 'data' => []];
            ...
            $message = ['status' => '0', 'message' => 'Store Not Registered', 'data' => []];
    ...
    public function storeprofile(Request $request)
    {
        ...
        if ($store) {
            $message = ['status' => '1', 'message' => 'Store Profile', 'data' => $data];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'Store not found', 'data' => []];
    ...
    public function storeupdateprofile(Request $request)
    {
        ...
        if ($chkstorphon && $chkstoremail) {
            ...
            $message = ['status' => '0', 'message' => trans('keywords.This Phone Number and Email Are Already Registered With Another Store')];
        ...
        if ($chkstorphon) {
            $message = ['status' => '0', 'message' => trans('keywords.This Phone Number is Already Registered With Another Store')];

            return $message;
        }
        if ($chkstoremail) {

            $message = ['status' => '0', 'message' => trans('keywords.This Email is Already Registered With Another Store')];
        ...
        if ($update) {
            ...
            $message = ['status' => '1', 'message' => 'Store Profile Updated', 'data' => $data];

            return $message;
        } else {
            $message = ['status' => '1', 'message' => 'Nothing to Update'];
    ...
    public function top_selling(Request $request)
    {
        ...
        if (count($topselling) > 0) {
            $message = ['status' => '1', 'message' => 'Top Products Of Store', 'total_orders' => $orders, 'total_revenue' => $sum, 'pending_orders' => $pending, 'data' => $topselling];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'nothing in top', 'data' => []];
