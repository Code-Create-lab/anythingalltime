...
class AssignController extends Controller
{
    ...
    public function delivery_boy_list(Request $request)
    {
        ...
        if (count($nearbydboy) > 0) {
            $message = ['status' => '1', 'message' => 'Delivery Boy List', 'data' => $nearbydboy];
    ...
    public function storeconfirm(Request $request)
    {
        ...
        if ($nearbydboy) {
            ...
            if ($orderconfirm) {
                ...
                $message = ['status' => '1', 'message' => 'order is confirmed and Assigned to '.$getDevice->boy_name];

                return $message;
            } else {
                $message = ['status' => '0', 'message' => 'Already Assigned to '.$getDevice->boy_name];

                return $message;
            }
        } else {
            $message = ['status' => '1', 'message' => 'no delivery boy is online/available'];
