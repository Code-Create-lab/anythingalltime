...
class DriverloginController extends Controller
{
    public function driver_login(Request $request)
    {
        ...
        if ($checkdriver1) {
            ...
            if ($checkdriver) {
                ...
                $message = ['status' => '1', 'message' => 'login successfully', 'data' => [$checkdriver]];
            ...
            $message = ['status' => '0', 'message' => 'Driver Not Registered', 'data' => []];
    ...
    public function driverprofile(Request $request)
    {
        ...
        if ($diver) {
            $message = ['status' => '1', 'message' => 'Delivery Boy Profile', 'total_incentive' => $total_incentive, 'received_incentive' => $received_incentive, 'remaining_incentive' => $remaining_incentive, 'driver_data' => $diver, 'bank_details' => $driver_bank];

            return $message;
        } else {
            $message = ['status' => '0', 'message' => 'Delivery Boy not found', 'data' => []];
    ...
    public function driverbank(Request $request)
    {
        ...
        if ($check) {
            ...
            if ($update) {
                $message = ['status' => '1', 'message' => 'Bank Details Updated Successfully'];
    ...
    public function driverupdateprofile(Request $request)
    {
        ...
        if ($chkboyrphon) {
            $message = ['status' => '0', 'message' => trans('keywords.This Phone Number Is Already Registered With Another Delivery Boy')];

            return $message;
        }

        // Check if there are actual changes to make
        $currentDriver = DB::table('delivery_boy')
            ->where('dboy_id', $dboy_id)
            ->first();

        if ($currentDriver &&
            $currentDriver->boy_name == $boy_name &&
            $currentDriver->boy_phone == $boy_phone &&
        ...
        if ($updated) {
            $driver = DB::table('delivery_boy')
                ->where('dboy_id', $dboy_id)
                ->first();
            $message = ['status' => '1', 'message' => 'Driver Profile Updated', 'data' => $driver];
