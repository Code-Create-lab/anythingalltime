...
class DriverorderController extends Controller
{
    ...
    public function delivery_out(Request $request)
    {
        ...
        if ($update) {
            $sms = DB::table('notificationby')
                ...
                ->first();
            $sms_status = $sms ? $sms->sms : 0;
            try {
                $sms_api_key = DB::table('msg91')
                    ->select('api_key', 'sender_id')
                    ->first();
                $api_key = $sms_api_key ? $sms_api_key->api_key : '';
                $sender_id = $sms_api_key ? $sms_api_key->sender_id : '';
            } catch (\Exception $e) {
                $api_key = '';
                $sender_id = '';
            }
            ...
            $email = DB::table('notificationby')
                ...
                ->where('user_id', $ord->user_id)
                ->first();
            $email_status = $email ? $email->email : 0;
            ...
            $message = ['status' => '1', 'message' => 'out for delivery'];
    ...
    public function delivery_completed(Request $request)
    {
        ...
        $getdboy = DB::table('delivery_boy')
            ...
            ->first();
        if (! $getdboy) {
            return ['status' => '0', 'message' => 'Delivery boy not found'];
        }
        ...
        $ph = DB::table('users')
            ...
            ->first();
        if (! $ph) {
            return ['status' => '0', 'message' => 'User phone not found'];
        }
        $user_phone = $ph->user_phone;
        $user_name = $ph->name;
        foreach ($var as $h) {
        ...
        $update = DB::table('orders')
            ->where('cart_id', $cart_id)
            ->update(['order_status' => $status, 'payment_status' => 'success', 'user_signature' => $filePath, 'dboy_incentive' => $in_am, 'delivery_date' => $dateet]);
        ...
        if ($update) {
            ...
            $message = ['status' => '1', 'message' => 'Delivery Completed'];
