...
trait SendMail
{
    public function payoutMail($amt, $store_name, $user_email, $app_name)
    {
        ...
        $data = ['to' => $user_email, 'from' => 'noreply@thecodecafe.in', 'to-name' => $store_name, 'from-name' => $app_name];
    ...
    public function codorderplacedMailstore($cart_id, $prod_name, $price2, $delivery_date, $time_slot, $user_email, $user_name, $store_n, $store_email)
    {
        ...
        $currency = DB::table('currency')
            ->first();
        $currency_sign = $currency->currency_sign;
        $data = ['to' => $store_email, 'from' => 'noreply@thecodecafe.in', 'to-name' => $store_n, 'from-name' => $app_name];
    ...
    public function codorderplacedMailadmin($cart_id, $prod_name, $price2, $delivery_date, $time_slot, $user_email, $user_name, $store_n, $admin_email, $admin_name)
    {
        ...
        $currency = DB::table('currency')
            ->first();
        $currency_sign = $currency->currency_sign;
        $data = ['to' => $admin_email, 'from' => 'noreply@thecodecafe.in', 'to-name' => $admin_name, 'from-name' => $app_name];
    ...
    public function sendrejectmail($cause, $user, $cart_id)
    {
        ...
        $currency = DB::table('currency')
            ->first();
        $currency_sign = $currency->currency_sign;
        $data = ['to' => $user->email, 'from' => 'noreply@thecodecafe.in', 'to-name' => $user->name, 'from-name' => $app_name];
    ...
    private function sendResetEmail($email, $token)
    {
        ...
        $data = ['to' => $email, 'from' => 'noreply@thecodecafe.in', 'to-name' => $email, 'from-name' => $app_name];
